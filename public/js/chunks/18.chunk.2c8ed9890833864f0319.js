  ...
  if (error.response) {
    ...
    for (var key in alert) {
      if (alert.hasOwnProperty(key)) {
        ...
        if (typeof alert[key] === 'number') {
          search[key] = alert[key];
        }
      }
    }

    console.log(search); // console.dir(alert.getBoundingClientRect())

    var top = 0; // window.scrollTo({ top, behavior: 'smooth' })
...
//# sourceMappingURL=18.chunk.2c8ed9890833864f0319.js.map